...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">Report Dipendenti</h1>
    </div>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('reports.dipendenti.generate') }}" method="POST">
                @csrf
                <div class="row mb-3">
                    <div class="col-md-12">
                        ...
                        <select class="form-control @error('worker_id') is-invalid @enderror" id="worker_id" name="worker_id" required>
                            ...
                            @foreach($workers as $worker)
                                <option value="{{ $worker->id }}" {{ old('worker_id') == $worker->id ? 'selected' : '' }}>
                                    ...
                                    @if($worker->worker_email) ({{ $worker->worker_email }}) @endif
                                </option>
                            @endforeach
                        </select>
                        @error('worker_id')
